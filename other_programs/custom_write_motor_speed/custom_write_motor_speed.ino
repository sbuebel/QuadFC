// LED should flash during startup, then turn off when ready
// If LED turns on during flight, there is an issue!
...
float angle_pitch_output, angle_roll_output, angle_yaw_output; // REAL VALUES
...
// ROLL stuff
float pid_p_gain_roll = 1.3;          // Increment by 0.2 until overcompensates, then decrease by 50%
float pid_i_gain_roll = 0.0;          // Increase by 0.01 until unstable, then decrease by 50%
float pid_d_gain_roll = 15;          // Raise until quad is unsteady, then decrease until quad is steady, then subtract 25%
...
float pid_i_gain_yaw = 0.2;
float pid_d_gain_yaw = 10;
int pid_max_yaw = 400;
...
Wire.begin();                                                           // Start I2C as master
  ...
  // wait until full throttle
  while (receiver_input[2] < 1900) {
    digitalWrite(status_led, HIGH);
  }
  Serial.println("Full Throttle Detected!");
  ...
  // wait until throttle comes back down
  while(receiver_input[2] > 1050) {
    digitalWrite(status_led, HIGH);
  }
  Serial.println("Low Throttle Detected!");


  delay(250);
  for (int i = 0; i < 1250; i ++) {
    PORTD |= B11110000;                                                     //Set digital poort 4, 5, 6 and 7 high.
    delayMicroseconds(receiver_input[2]);                                   //Set LOW threshold. -> SHOULD BE 1000!
    PORTD &= B00001111;                                                     //Set digital poort 4, 5, 6 and 7 low.
    delayMicroseconds(4000 - receiver_input[2]);
  }
  ...
  for (int cal_int = 0; cal_int < 2000 ; cal_int ++) {                  //Run this code 2000 times
    if (cal_int % 125 == 0) {
      led_on = !led_on;                                                 //Print a dot every 125 readings
    }
    if (led_on == true) {
      digitalWrite(status_led, HIGH);
    } else {
...
void loop() {
  ...
  // MAKE SURE WE STAY WITHIN 4000 MICROSECONDS, IF NOT, ANGLE CALCULATIONS BREAK!!!
  ...
  // Get controller setpoint! Need this for PID calculations

  // SETPOINT is in degrees/second!!!
  pid_roll_setpoint = 0;
  if (receiver_input[0] > 1508) { // ROLL ROLL ROLL
    pid_roll_setpoint = (receiver_input[0] - 1508); // This will be positive!
  } else if (receiver_input[0] < 1492) {
    pid_roll_setpoint = (receiver_input[0] - 1492); // This will be negative!
  }
  pid_roll_setpoint -= angle_roll_output*15;
  pid_roll_setpoint /= 3;
  ...
  pid_pitch_setpoint -= angle_pitch_output*15;
  ...
  //print_angles();
  //Serial.println(0.001*(micros() - loop_timer));

  // Actually update motor speeds here
  if (start == 2) {
    ...
    // Keep motors running
    if (esc_1 < 1270) esc_1 = 1270;
    if (esc_2 < 1270) esc_2 = 1270;
    if (esc_3 < 1270) esc_3 = 1270;
    if (esc_4 < 1270) esc_4 = 1270;
  ...
  if (micros() - loop_timer > 4050) digitalWrite(status_led, HIGH); // We are taking too long!!!
  ...
  // while waiting, get gyro signals, why not?
  gyro_signal_in();
  
  while(PORTD >= 16) {                                                       //Stay in this loop until output 4,5,6 and 7 are low.
    esc_loop_timer = micros();                                              //Read the current time.
    //Serial.println(esc_loop_timer);
  ...
  int esc_1 = receiver_input[2];
  int esc_2 = receiver_input[2];
  int esc_3 = receiver_input[2];
  int esc_4 = receiver_input[2];
...
void gyro_signal_in() {
  ...
  angle_roll_acc += 0.89;                                                // Accelerometer calibration value for pitch
  angle_pitch_acc -= 1.145;                                               // Accelerometer calibration value for roll
